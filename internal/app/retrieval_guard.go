...
func buildRetrievalProgress(events []harnessruntime.Event) RetrievalProgress {
	...
	for _, event := range events {
		...
		updateRetrievalProgress(&progress, toolName, result)
	}
	return progress
}

func updateRetrievalProgress(progress *RetrievalProgress, toolName string, result map[string]any) {
	switch toolName {
	case "web.search":
...
func decideRetrievalProgress(progress RetrievalProgress, nextAction model.Action) RetrievalDecision {
